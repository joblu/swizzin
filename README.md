...
Using `wget`:
```shell
bash <(wget -qO - https://raw.githubusercontent.com/joblu/swizzin/master/setup.sh) && . ~/.bashrc
```

Using `curl`:
```shell
bash <(curl -sL https://raw.githubusercontent.com/joblu/swizzin/master/setup.sh) && . ~/.bashrc
```
